...
await test("auth: /api/meta rejects missing and bad tokens, accepts owner", async () => {
  ...
  assert.equal(ok.json.version, 14);
...
await test("roles: view keeps the classic page; comment/edit get the app shell", async () => {
  ...
  assert.ok(commentPage.text.includes("/__web/14/app.js"));
  assert.equal((await call("/__web/14/app.js")).status, 503);
...
await test("mermaid: static pages hydrate diagram blocks, shell knows the module URL", async () => {
  ...
  assert.ok(withDiagram.text.includes("/__web/14/mermaid.js"));
  ...
  assert.ok(shell.text.includes(`window.__DK_MERMAID_URL = "/__web/14/mermaid.js"`));
  assert.equal((await call("/__web/14/mermaid.js")).status, 503);
