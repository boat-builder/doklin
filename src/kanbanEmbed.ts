// A board inside a note: the ` ```kanban ` fence, as a ProseMirror node.
//
//   ```kanban
//   store: ./Projects
//   ```
...
//
//   - a $remark transform retypes the `code` mdast node whose lang is exactly
//     `kanban` into a `kanbanEmbed` node at parse time, and writes it back as
//     the same fence at serialize time. The code-block schema therefore never
...
//   - a $nodeSchema for an ATOM block node whose only attribute is the fence's
//     raw config text. An atom has no editable content, so the document's
//     text and the board's state can never be confused for one another;
//   - a $view node view mounting the React frame (KanbanEmbed.tsx) inside a
...
const MDAST_TYPE = "kanbanEmbed";

/* ---------- the schema ---------- */

export const kanbanEmbedSchema = $nodeSchema("kanban_embed", () => ({
  group: "block",
  atom: true,
  selectable: true,
  draggable: true,
  isolating: true,
  attrs: { config: { default: "", validate: "string" } },
  parseDOM: [
    {
  ...
  parseMarkdown: {
    ...
    runner: (state: any, node: any, type: any) => {
      state.addNode(type, { config: typeof node.value === "string" ? node.value : "" });
    },
  },
  toMarkdown: {
...
// Retype every ```kanban fence in the tree. Recursive because an embed is
// legal wherever a code block is — inside a list item, inside a quote.
function claimKanbanFences(node: any): void {
  if (!node || !Array.isArray(node.children)) return;
  for (const child of node.children) {
    if (child?.type === "code" && isKanbanFence(child.lang, child.meta)) {
      child.type = MDAST_TYPE;
    } else {
      claimKanbanFences(child);
    }
  }
}
...
export const kanbanEmbedRemark = $remark("kanbanEmbed", () => {
  return function (this: any) {
    ...
    toMarkdownExtensions.push({
      handlers: {
        [MDAST_TYPE](node: any) {
          return fenceKanban(typeof node.value === "string" ? node.value : "");
        },
      },
    });
    return (tree: any) => {
      claimKanbanFences(tree);
    };
  };
});
...
const liveViews = new Set<KanbanEmbedNodeView>();

/** Redraw every mounted embed — call when the host or read-only state moves. */
export function refreshKanbanEmbeds(): void {
  for (const v of liveViews) v.redraw();
}

class KanbanEmbedNodeView implements NodeView {
  dom: HTMLElement;
  private root: Root;
  ...
  private writeConfig(next: string): void {
    ...
    this.view.dispatch(
      this.view.state.tr.setNodeMarkup(pos, undefined, { config: next }),
    );
  }
...
/**
 * The slash menu's Board item. Inserts an embed with an EMPTY config — the
 * frame then asks which board, which is a question better answered in place
 * than in a modal.
 */
export function insertKanbanEmbed(ctx: Ctx): void {
  // Clear the "/board" the user typed, the way the Diagram item does.
  ctx.get(commandsCtx).call(clearTextInCurrentBlockCommand.key);
  const view = ctx.get(editorViewCtx);
  const type = kanbanEmbedSchema.type(ctx);
  const { state } = view;
  const { $from } = state.selection;
  const tr = state.tr;
  const node = type.create({ config: "" });
